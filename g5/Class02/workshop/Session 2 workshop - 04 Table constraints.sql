...
ALTER TABLE [dbo].[Product] WITH CHECK
ADD CONSTRAINT UC_Product_Name_ UNIQUE (Name)
GO

-- Try to find all the duplicates. This command will re
select  Name
from Product
group by name 
having count(*)>1
-- After you find the names of the duplicate values try to select the rows ans get the Id of one of the to update it
